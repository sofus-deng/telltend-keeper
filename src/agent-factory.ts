...
export function createKeeperAgent(options: KeeperAgentOptions = {}): Agent {
  ...
  const modelId = options.modelId ?? process.env.KEEPER_BEDROCK_MODEL_ID ?? 'global.anthropic.claude-sonnet-4-6'
  const region = options.region ?? process.env.AWS_REGION ?? process.env.AWS_DEFAULT_REGION ?? 'us-west-2'
...
5. Always verify the actual fixture state after publication.
...
Never equate a publish tool's success response with a verified result.`,
    tools: [
      inspectSite,
      proposeChange,
      validateChange,
